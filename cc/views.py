...
def addcomplaint(request):
    ...
    if request.method == 'POST':
        form = Complaint_form(request.POST, request.FILES)  # Added request.FILES for handling file uploads
        if form.is_valid():
            f1 = form.save(commit=False)
            f1.owner = request.user
            f1.save()
            form = Complaint_form()  # Resets the form after successful submission
...
def mydetailed(request, id):
    try:
        # Use filter to fetch all feedback entries related to the complaint with the given ID
        feed = Feedback.objects.filter(complaint_id=id)
    except Feedback.DoesNotExist:
        feed = None

    # Retrieve the complaint with the given ID
    ob = Complaint.objects.get(pk=id)
    
    # Render the template with both complaint and feedback data
...
def download_report(request, id):
    ...
    feedback = Feedback.objects.filter(complaint=complaint).first()  # Adjust as necessary

    title = complaint.title
    description = complaint.description
    priority = complaint.priority
    attachment_url = complaint.attachment.url if complaint.attachment else 'No attachment'
    feedback_content = feedback.feedback if feedback else "No feedback provided."
    ...
    # Create the HttpResponse to serve the file as a download
    response = HttpResponse(report_content, content_type='text/plain')
    response['Content-Disposition'] = f'attachment; filename="complaint_{id}_report.txt"'
...
def feedback_page(request, id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            feedback.save()
            return redirect('mydetailed', id=id)
    else:
        form = FeedbackForm()
    return render(request, 'feedback_form.html', {'form': form, 'complaint': complaint})
from django.shortcuts import render
from django.db.models import Count
from .models import Complaint

def complaints_chart(request):
    # Data for the chart: Count complaints by category
    data = Complaint.objects.values('category__category').annotate(total=Count('id'))
    categories = [item['category__category'] for item in data]
    counts = [item['total'] for item in data]

    # Render the chart template
    return render(request, 'admin/complaints_chart.html', {
